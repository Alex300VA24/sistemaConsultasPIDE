const Dashboard = {
    ...
    initSessionTimeout() {
        const cfg = window.SESSION_CONFIG;
        if (!cfg || !cfg.loginTime || !cfg.timeout) return;
        if (!document.getElementById('dashboardContainer')) return;

        const remaining = (cfg.loginTime + cfg.timeout) - Math.floor(Date.now() / 1000);
        ...
        setTimeout(() => this.redirectToLogin(true), remaining * 1000);
    },

    redirectToLogin(sesionExpirada = false) {
        try {
            Storage.clearAuth();
        } catch (e) {
            console.error('Error limpiando sesión:', e);
        }
        window.location.href = Constants.ROUTES.LOGIN + (sesionExpirada ? '?sesion_exp=1' : '');
    },
...
// Solo definir toggleSubmenu si el sidebar no definió ya una versión avanzada
