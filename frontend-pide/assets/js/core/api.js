class ApiClient {
    ...
    async request(endpoint, options = {}) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                // Sesión expirada o no autenticado: redirigir al login automáticamente
                if (response.status === 401 && !this.isLoginPage()) {
    ...
    isLoginPage() {
        return !!document.getElementById('formLogin') || window.location.pathname.endsWith('/login');
    }

    redirectToLogin(sesionExpirada = false) {
        try {
            if (window.Storage && typeof window.Storage.clearAuth === 'function') {
